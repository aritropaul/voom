...
enum DeployStepStatus: Equatable {
    case pending
    case inProgress
    case completed
    case skipped(String)
...
struct DeployStep: Identifiable, Equatable {
    let id: String
    let label: String
    var status: DeployStepStatus = .pending
}

@Observable
@MainActor
final class DeployProgress {
    ...
    func updateStep(_ id: String, status: DeployStepStatus) {
...
enum CloudflareDeployError: LocalizedError {
...
actor CloudflareDeployService {
    static let shared = CloudflareDeployService()
