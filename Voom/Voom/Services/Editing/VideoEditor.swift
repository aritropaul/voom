...
actor VideoEditor {
    ...
    func trim(sourceURL: URL, startTime: CMTime, endTime: CMTime, outputURL: URL) async throws {
    ...
    func cutSections(sourceURL: URL, removals: [CMTimeRange], outputURL: URL) async throws {
        let asset = AVURLAsset(url: sourceURL)
        let duration = try await asset.load(.duration)

        let composition = AVMutableComposition()

        // Calculate keep ranges (inverse of removals)
    ...
    func stitch(sourceURLs: [URL], outputURL: URL) async throws {
    ...
    func adjustTranscript(segments: [TranscriptEntry], removals: [CMTimeRange]) -> [TranscriptEntry] {
        ...
        for var segment in segments {
            ...
            // Calculate cumulative offset from prior removals
